import itertools
from typing import Iterable, TypeVar, Dict, Tuple, Set, Optional, List

from confectioner import mix
from confectioner.templating import set_dotted_key

from .types import Evaluatable, JSONDict, JSONType, EvaluationError, ValidationError
...
class Map(Evaluatable[Iterable[A]]):
    ...
    def _update_options(options: JSONDict, *args: Tuple[str, JSONType]) -> JSONDict:
        ...
        for key, value in args:
            set_dotted_key(key, value, new_options)
    ...
    def _iterate_over_option(
            key: str,
            iterable: Evaluatable[Iterable[JSONType]],
            options: JSONDict
    ...
    def over(self, iterables: Dict[str, Evaluatable[Iterable[JSONType]]]) -> 'Map[A]':
        ...
        ----------
            iterables : Dict[str, Evaluatable[Iterable[JSONType]]]
                A dictionary of iterables to map over. Keys are the which entries in the options dictionary
                to update, and values are the iterables to iterate over. If the key is dotted, the value will
                be nested in the options dictionary.
